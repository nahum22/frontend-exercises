...
class CarAgencyManager {
  ...
  addCarToAgency(agencyId, car) {
    for (let i = 0; i < this.agencies.length; i++) {
      if (this.agencies[i].agencyId == agencyId) {
        this.agencies[i].cars[0].models.push(car);
  ...
  changeAgencyCashOrCredit(agencyId, cashOrCredit) {
    for (let i = 0; i < this.agencies.length; i++) {
      if (
        this.agencies[i].agencyId === agencyId &&
        typeof cashOrCredit === "number"
      ) {
        this.agencies[i].cash += cashOrCredit;
        return true;
      }
    }
    return false;
    ...
    for (let i = 0; i < this.agencies.length; i++) {
      if (this.agencies[i].agencyId == agencyId) {
        for (let i1 = 0; i1 < this.agencies[i].cars.length - 1; i1++) {
          for (let i3 = 0; i3 < this.agencies[i].cars[i1].models.length; i3++) {
            if (this.agencies[i].cars[i1].models[i3].carNumber === carId) {
              this.agencies[i].cars[i1].models[i3].price = newPrice;
              return true;
            }
          }
        }
  ...
  getTotalAgencyRevenue(agencyId) {
    for (let i = 0; i < this.agencies.length; i++) {
      if (this.agencies[i].agencyId == agencyId) {
        return this.agencies[i].cash + this.agencies[i].credit;
      }
    }
  }
  ...
  transferCarBetweenAgencies(fromAgencyId, toAgencyId, carId) {
    let result1 = this.addCarToAgency(toAgencyId, carId);

    let result2 = this.removeCarFromAgency(fromAgencyId);
...
class CustomerManager {
  ...
  searchCustomer(idOrName) {
    for (let customer of this.customers) {
      if (customer.id === idOrName || customer.name === idOrName) {
        return customer;
      }
    }
    return false;
...
class CarManager {
  ...
  getAllCars() {
    ...
    for (let i = 0; i < this.agencies.length; i++) {
      for (let i1 = 0; i1 < this.agencies[i].cars.length; i1++) {
        for (let i3 = 0; i3 < this.agencies[i].cars[i1].models.length; i3++) {
          let brandname = this.agencies[i].cars[i1].brand;
          let temp = this.agencies[i].cars[i1].models[i3];
          result.push([temp, brandname]);
        }
      }
    }
  ...
  searchCars(year, price, brand = null) {
    ...
    for (let car of cars) {
      if (
        ...
        if (car[0].year === year && car[0].price == price && car[1] == brand) {
          result.push(car[0]);
        }
      }
    }
  ...
  getMostExpensiveCar() {
    let tempo = this.getAllCars();

    tempo.sort((a, b) => {
      let price1 = a[0].price;
      let price2 = b[0].price;
...
class CarPurchaseManager {
  ...
  findCarById(carId) {
    for (let i = 0; i < this.agencies.length; i++) {
      for (let i1 = 0; i1 < this.agencies[i].cars.length - 1; i1++) {
        for (let i3 = 0; i3 < this.agencies[i].cars[i1].models.length; i3++) {
          if (this.agencies[i].cars[i1].models[i3].carNumber === carId) {
            return this.agencies[i].cars[i1].models[i3];
          }
        }
      }
    }
  ...
  sellCar(carId, customerId) {
    let car = this.findCarById(carId);
    if (car == false) {
      return "the vhiecle does not exist in the agency";
    }
    const customerManager = new CustomerManager(customers);
    const agencyManager = new CarAgencyManager(agencies);

    let customer = customerManager.searchCustomer(customerId);

    if (customer.cash > car.price) {
      customer.cash = customer.cash - car.price;
      let agencyId = car.ownerId;

      agencyManager.removeCarFromAgency(agencyId, car.carNumber);
      agencyManager.changeAgencyCashOrCredit(agencyId, car.price);
      car.ownerId = customerId;
      customer.cars.push(car);

      this.numberOfTransactions += 1;
      this.sumOfAllTransactions += car.price * 1.17;
      this.totalTaxesPaid += 0.17 * car.price;
      return true;
    }
  ...
  getTotalMarketRevenue() {
    const agencyManager = new CarManager(agencies);
    let result = 0;
    for (let car of agencyManager.getAllCars()) {
      if (typeof car[0].price !== "undefined") {
        result += car[0].price;
      }
    }
    return result;
...
let carAgencyManager = new CarAgencyManager(agencies);
// // Test searchAgency
console.log(
  "Search Agency by ID: ",
...
console.log(
  "Add Car to Agency: ",
  carAgencyManager.addCarToAgency("Plyq5M5AZ", newCar.models[0])
);
...
console.log("Search Cars (No Brand): ", carManager.searchCars(2020, 966500));
